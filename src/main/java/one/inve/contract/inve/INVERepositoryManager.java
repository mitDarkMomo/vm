...
public class INVERepositoryManager {
    ...
    public static INVERepositoryRoot getRepoRoot(String dbId) {
        String dbPath = PathUtils.getDataFileDir();
        String cfgDir = dbPath + "configdata/";
        return getRepoRoot(cfgDir, dbPath, dbId);
    }

    public static INVERepositoryRoot getRepoRoot(String cfgDir, String dbPath, String dbId) {
        
        DbSource<byte[]> stateDS = dsMap.get(dbId + "state");
        if(stateDS == null) {
            stateDS = blockchainDB(dbPath, dbId + "state");
            dsMap.put(dbId + "state", stateDS);
        }

        DbSource<byte[]> receiptDS = dsMap.get(dbId + "receipt");
        if(receiptDS == null) {
            receiptDS = blockchainDB(dbPath, dbId + "receipt");
    ...
    private static DbSource<byte[]> keyValueDataSource(String dbPath, String name, DbSettings settings) {
        try {
            DbSource<byte[]> dbSource;
            dbSource = rocksDbDataSource(dbPath, name);
            dbSource.init(settings);
            logger.debug("DbSource name {} set", dbSource.getName());
            return dbSource;
        } finally {
            logger.info("key-value data source created: " + name);
        }
    }
    ...
    private static INVERocksDbDataSource rocksDbDataSource(String dbPath, String name) {
        return new INVERocksDbDataSource(dbPath, name);
    }
    ...
    private static DbSource<byte[]> blockchainDB(String dbPath, String name) {
        DbSettings settings = DbSettings.newInstance()
                .withMaxThreads(Math.max(1, Runtime.getRuntime().availableProcessors() / 2));
        return keyValueDataSource(dbPath, name, settings);
    }
}
